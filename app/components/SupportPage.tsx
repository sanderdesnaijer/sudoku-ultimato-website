  ...
  const breadcrumbStructuredData = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        item: `https://sudokuultimato.metsander.com${
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans selection:bg-purple-500 selection:text-white flex flex-col">
      ...
      <header className="py-6 px-6 border-b border-white/5 flex justify-between items-center">
        <div className="max-w-7xl">
          <Link
            href={getLocalizedHomePath()}
            className="text-sm font-semibold text-neutral-400 hover:text-white transition-colors flex items-center gap-2"
          >
            ← {t.hero.title}
          </Link>
        </div>
          ...
          <div className="bg-neutral-900/50 backdrop-blur-md rounded-3xl p-8 border border-white/10 shadow-2xl">
            <p className="text-neutral-300 mb-6 text-center leading-relaxed">
              ...
              <a
                href={`mailto:${SUPPORT_EMAIL}?subject=${encodeURIComponent(
                  t.footer.contactEmailSubject
                )}`}
                className="text-white hover:text-purple-400 underline underline-offset-4 decoration-white/20 hover:decoration-purple-400/50 transition-all font-medium"
              >
                {SUPPORT_EMAIL}
              </a>
            ...
            <div className="flex justify-center">
              <a
                ...
                className="inline-flex items-center gap-3 px-6 py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl font-medium transition-all duration-200 border border-white/5 hover:border-white/20"
              >
                📧 {SUPPORT_EMAIL}
              </a>
            </div>
      ...
      <footer className="py-12 border-t border-white/5 text-center mt-12 bg-neutral-950">
        <nav className="flex justify-center gap-8 mb-8 flex-wrap">
          <Link
            href={lang === DEFAULT_LANG ? "/support/" : `/${lang}/support/`}
            className="text-white transition-colors text-sm uppercase tracking-widest"
          >
            {t.footer.support}
          </Link>
          <Link
            href={lang === DEFAULT_LANG ? "/privacy/" : `/${lang}/privacy/`}
